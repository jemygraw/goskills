...
type SkillPackage struct {
	...
	Body      string         `json:"body"` // Markdown content of SKILL.md
...
func extractAndParseSKILLmd(data []byte) (SkillMeta, string, error) {
	marker := []byte("---")
	var meta SkillMeta
	var body string
...
func ParseSkillPackage(dirPath string) (*SkillPackage, error) {
	...
	meta, body, err := extractAndParseSKILLmd(mdContent)
	if err != nil {
		return nil, err
	}
